#!/bin/sh
# Flash WFB-NG firmware onto the drone from the local cache.
# Run firmware_download_wfbng.sh first to download the latest firmware.
...
if [ ! -f "${CACHED_TGZ}" ]; then
    ...
    echo "[gs]   ${CACHED_TGZ}"
    echo "[gs] Run firmware_download_wfbng.sh first."
    exit 1
fi
